...
const Arena=(()=>{
  ...
  function draw(){
    ...
    const tFX=W*.5+Math.max(-W*.28,Math.min(W*.28,(diff/mx)*W*.24));
    if(sFlagX<0)sFlagX=tFX;
    sFlagX+=(tFX-sFlagX)*.09;
    drawFlag(sFlagX);
  ...
  function drawM1(){
    ...
    const mv=W*.32;
    const tx1=W*.14+(p1.score/20)*mv;
    const tx2=W*.86-(p2.score/20)*mv;
    // Smooth lerp — no jerky jumps when score changes
    if(sX1<0)sX1=tx1;
    if(sX2<0)sX2=tx2;
    sX1+=(tx1-sX1)*.09;
    sX2+=(tx2-sX2)*.09;
    const x1=sX1,x2=sX2;
    const gndY=H*.84;
    // Edge scale: characters shrink as they approach arena boundary
    const eS1=Math.max(.18,Math.min(1,(x1/W)*9,((W-x1)/W)*9));
    const eS2=Math.max(.18,Math.min(1,(x2/W)*9,((W-x2)/W)*9));
    ...
    drawHuman(x1,gndY-H*.38,'#F97316','#C2410C',SK[0],HR[0],false,p1s,0,eS1);
    if(snap.fallenSide==='left')drawDust(x1,gndY,snap.fallProgress);
    drawHuman(x2,gndY-H*.38,'#1E40AF','#1E3A8A',SK[1],HR[1],true,p2s,0,eS2);
